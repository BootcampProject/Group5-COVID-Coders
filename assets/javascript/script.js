$(document).ready(function () {
  $("#journal").on("click", function (event) {
    ...
    var urlCovid =
      "https://api.springernature.com/metadata/json?q=keyword:covid&api_key=6dc88c6bf4873b8f5d8482c671f650cc";
    ...
    $.ajax(stats).done(function (response) {
      ...
      $("#newDeath").append(
        `<div>New Deaths: ${response.stats.newDeaths}</div>`
      );
      $("#newDeath").append(`<hr class="uk-description-list-divider">`);
      $("#newConfirmed").append(
        `<div>New Confirmed Cases: ${response.stats.newlyConfirmedCases}</div>`
      );
      $("#newConfirmed").append(`<hr class="uk-description-list-divider">`);
      $("#newRecovered").append(
        `<div>New Recovered Cases: ${response.stats.newlyRecoveredCases}</div>`
      );
      $("#newRecovered").append(`<hr class="uk-description-list-divider">`);
      $("#totalConfirmed").append(
        `<div>Total Confirmed Cases: ${response.stats.totalConfirmedCases}</div>`
      );
      $("#totalConfirmed").append(`<hr class="uk-description-list-divider">`);
      $("#totalDeaths").append(
        `<div>Total Deaths: ${response.stats.totalDeaths}</div>`
      );
      $("#totalDeaths").append(`<hr class="uk-description-list-divider">`);
      $("#totalRecovered").append(
        `<div>Total Recovered Cases: ${response.stats.totalRecoveredCases}</div>`
      );
    });
